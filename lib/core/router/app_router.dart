...
final appRouterProvider = Provider<GoRouter>((ref) {
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: '/hire-requests', builder: (_, __) => const HireRequestsScreen()),
      GoRoute(
        path: '/profile/:userId',
        builder: (context, state) {
    ...
    redirect: (context, state) async {
      ...
      if (path == '/auth' || path == '/account-type') return null;
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return null;
      final accountType = await ref.read(accountTypeServiceProvider).getAccountType();
